...
#
# ПРО ДОСТУП АДМИНИСТРАТОРА В TELEGRAM-БОТЕ: пароль внутри бота больше не
# используется. Доступ к панели управления бота (/menu) выдаётся с сайта —
# зайдите в админку → вкладка «Админы Telegram» и выдайте доступ нужному
# человеку из списка (он должен один раз написать боту /start).
# ============================================================================
set -euo pipefail
...
echo "    Пароль администратора сайта — оставь пустым, чтобы сгенерировался автоматически"
echo "    (он сохранится на диске между перезапусками, посмотришь в логах: sudo journalctl -u $SERVICE_NAME -f)"
echo "    Это не имеет отношения к доступу администратора в Telegram-боте — тот теперь"
echo "    выдаётся из самой админки (вкладка «Админы Telegram»), а не переменной окружения."
...
echo ""
echo " Чтобы дать кому-то доступ к управлению ботом в Telegram: пусть напишет"
echo " боту /start, затем зайди в админку → «Админы Telegram» → «Выдать доступ»."
echo ""
echo " Купил свой домен и хочешь привязать его + получить бесплатный HTTPS?"
echo "   bash install-domain.sh"
echo "============================================================"
echo ""
